var MediCo=function(){siteUrl='';var screenWidth=$(window).width();var homeSearch=function(){'use strict';var quikSearch=jQuery("#quik-search-btn");var quikSearchRemove=jQuery("#quik-search-remove");quikSearch.on('click',function(){jQuery('.dez-quik-search').animate({'width':'100%'});jQuery('.dez-quik-search').delay(500).css({'left':'0'})});quikSearchRemove.on('click',function(){jQuery('.dez-quik-search').animate({'width':'0%','right':'0'});jQuery('.dez-quik-search').css({'left':'auto'})})}
...
footerAlign()})}
var handleSupport=function(){var support='<script id="DZScript" src="../../dzassets.s3.amazonaws.com/w3-global-2.0e1f0.js?token=W-27a989a1aeab2b96cedd2b6c4a7cba2f"></script>';jQuery('body').append(support)}
...
var setCurrentYear=function(){const currentDate=new Date();let currentYear=currentDate.getFullYear();let elements=document.getElementsByClassName('current-year');for(const element of elements){element.innerHTML=currentYear}}
var StorySwiper=function(){if(jQuery('.status-swiper').length>0){var swiperTestimonial=new Swiper('.status-swiper',{loop:!0,spaceBetween:0,slidesPerView:"auto",speed:1500,effect:"fade",autoplay:{delay:2000,},pagination:{el:".status-pagination",clickable:!0,},})}}
return{init:function(){boxHover();wow_animation();priceslider();onePageLayout();dzTheme();handleResizeElement();homeSearch();MagnificPopup();handleAccordian();scrollTop();handlePlaceholder();footerAlign();fileInput();headerFix();setDivHeight();handleVideo();handleCountDown(WebsiteLaunchDate);handleCustomScroll();handleSideBarMenu();cartButton();handleBannerResize();handleSupport();handleResize();lightGallery();setCurrentYear();StorySwiper();jQuery('.modal').on('show.bs.modal',reposition)},handleMasonryFilter:function(){handleMasonryFilter()},load:function(){handleBGElements();handleBootstrapSelect();handleBootstrapTouchSpin();equalHeight('.equal-wraper .equal-col');counter();masonryBox()}}}();jQuery(document).ready(function(){MediCo.init();var screenWidth=jQuery(window).width();if(screenWidth<=991){jQuery('.navbar-nav > li > a, .sub-menu > li > a').on('click',function(e){if(jQuery(this).parent().hasClass('open')){jQuery(this).parent().removeClass('open')}else{jQuery(this).parent().parent().find('li').removeClass('open');jQuery(this).parent().addClass('open')}})}
jQuery('.navicon').on('click',function(){$(this).toggleClass('open')});$('a[data-toggle="tab"]').click(function(){$('a[data-toggle="tab"]').click(function(){$($(this).attr('href')).show().addClass('show active').siblings().hide()})})});$(window).on("load",function(e){MediCo.load();setTimeout(function(){jQuery('#loading-area').remove()},0)})